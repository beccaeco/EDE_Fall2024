...
gage_data <- read.table(
  theURL,
  skip = 26, #number of lines to skip
  header=TRUE,
  sep='\t', #/t means tab
  stringsAsFactors = T) 
...
neuse <- renameNWISColumns(neuse); #data retrieval package specifically for nwis data
colnames(neuse)
...
```{r}
#Apply the API key -- just need to install once
...
#american community survey -- tidy census package
...
electricityprices <- data.frame(State = the_states,
                             Average_Price = as.numeric(the_price),
