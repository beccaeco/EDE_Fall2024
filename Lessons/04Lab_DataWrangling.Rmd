...
> Add notes here: we can see changes/trends over longer periods of time
...
summary(NTL.phys.data.PeterPaul$lakename)
...
> Answer: they might change? if we edit the table
...
NTL.phys.data.longer <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")


#Exercise 6: Spread temperatureC into more than one column based on the depth.

NTL.phys.data.wider <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C, id_cols = sampledate)

NTL.phys.data.PeterPaul.wider <- pivot_wider(NTL.phys.data.PeterPaul, names_from = depth, values_from = temperature_C, id_cols = sampledate)
